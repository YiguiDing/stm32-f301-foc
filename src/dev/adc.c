...
uint16_t adc_value[ADC_CHANNEL_NUM] = {0};
void (*adc_on_update)() = NULL;
void adc_init()
{
    adc_on_update = NULL;
    /* GPIO配置 */
    ...
    NVIC_InitTypeDef NVIC_InitStruct = {
        ...
        .NVIC_IRQChannelPreemptionPriority = 1, // 抢占优先级,0最高优先级
    ...
    DMA_InitTypeDef DMA_InitStruct =                   // 配置DMA
        {
            ...
            .DMA_BufferSize = ADC_CHANNEL_NUM,                         // 数据长度3
    ...
    /* ADC配置 */
    // ADCCLK最大频率不能超过14Mhz，当主频为72M时，ADC预分频器最小只能设置6即12M
    // 时钟源       预分频          采样周期         采样通道       采样频率
    // 72M            /6              /1.5            /3            2.81M
    // 72M            /6              /2.5            /3            1.686M
    // 72M            /6              /4.5            /3            0.88M=888.88khz
    // 72M            /6              /7.5            /3            533.33khz
    // 72M            /6              /19.5           /3            205.12khz
    // 72M            /16             /1.5            /3            1.055M
    // 72M            /64             /1.5            /3            263.888k
    // 72M            /128            /1.5            /3            131.944k
    // 72M            /128            /4.5            /3            43.981k
    // 72M            /128            /7.5            /3            26.388k
    // 72M            /128            /19.5           /3            10.149k
    // 72M            /128            /181.5          /3            1.090k
    RCC_ADCCLKConfig(RCC_ADC12PLLCLK_Div6);                                    // ADC预分频器
    RCC_AHBPeriphClockCmd(RCC_AHBPeriph_ADC12, ENABLE);                        // 开启ADC时钟
    ADC_RegularChannelConfig(ADC1, ADC_Channel_1, 1, ADC_SampleTime_1Cycles5); // 通道1放入序列1，采样时间为1.5个时钟周期
    ADC_RegularChannelConfig(ADC1, ADC_Channel_2, 2, ADC_SampleTime_1Cycles5); // 通道2放入序列2，采样时间为1.5个时钟周期
    ADC_RegularChannelConfig(ADC1, ADC_Channel_3, 3, ADC_SampleTime_1Cycles5); // 通道3放入序列3，采样时间为1.5个时钟周期
    ...
    ADC_InitTypeDef ADC_InitStruct = // 配置ADC
        {
            ...
            .ADC_ExternalTrigConvEvent = ADC_ExternalTrigConvEvent_9,          // 外部触发源（TIM1 TRGO (STM32F30x)）
            ...
            .ADC_NbrOfRegChannel = ADC_CHANNEL_NUM,                            // 通道数
        };
    ...
    // 中断配置
    // ADC_ITConfig(ADC1, ADC_IT_EOS, ENABLE); // 序列转换完成中断: 在中断函数 ADC1_2_IRQHandler() 中处理
...
#include "serial.h"
// void __attribute__((weak)) adc_on_update()
// {
//     // serial_printf(
//     //     "adc1: %.6f,%.6f,%.6f\n",

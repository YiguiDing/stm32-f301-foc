...
void serial_init()
{
    ...
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource3, GPIO_AF_7); // USART2_TX
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource4, GPIO_AF_7); // USART2_RX
    ...
    NVIC_InitTypeDef NVIC_InitStruct = {
        ...
        .NVIC_IRQChannel = USART2_IRQn,         // 中断通道: USART2_IRQn
    ...
    DMA_InitTypeDef DMA_Init_USART2_TX =               // 配置DMA
        {
            .DMA_PeripheralBaseAddr = (uint32_t)&(USART2->TDR),      // 外设起始地址
    ...
    DMA_InitTypeDef DMA_Init_USART2_RX = // 配置DMA
        {
            .DMA_PeripheralBaseAddr = (uint32_t)&(USART2->RDR),      // 外设起始地址
    ...
    DMA_Init(DMA1_Channel7, &DMA_Init_USART2_TX); // 初始化DMA
    DMA_Init(DMA1_Channel6, &DMA_Init_USART2_RX); // 初始化DMA
    DMA_Cmd(DMA1_Channel7, DISABLE);              // 关闭DMA发送搬运（按需开启）
    DMA_Cmd(DMA1_Channel6, ENABLE);               // 启用DMA接收搬运（始终开启）
    ...
    /* USART.NVIC配置 */
    USART_ITConfig(USART2, USART_IT_IDLE, ENABLE); // 使能溢出中断、空闲中断
    /* USART.DMA 配置 */
    USART_DMACmd(USART2, USART_DMAReq_Tx | USART_DMAReq_Rx, ENABLE); // 使能DMA收发请求
    /*启动USART*/
...
void USART2_IRQHandler()
{
    if (USART_GetITStatus(USART2, USART_IT_IDLE) == SET)
    {
        ...
        uint8_t *data = (uint8_t *)DMA1_Channel6->CMAR;                            // 获取内存起始地址
        uint16_t len = MAX_RX_BUFFER_SIZE - DMA_GetCurrDataCounter(DMA1_Channel6); // 获取数据长度
        DMA1_Channel6->CMAR = (uint32_t)serial_next_rx_buffer();                   // 切换双缓冲区
        DMA_SetCurrDataCounter(DMA1_Channel6, MAX_RX_BUFFER_SIZE);                 // 设置DMA接收最大数据长度
        DMA_ClearFlag(DMA1_FLAG_TE6);                                              // 清除错误标志（否则无法重启DMA） "This EN bit cannot be set again by software to reactivate the channel x, until the TEIFx bit of the DMA_ISR register is set."
        USART_ClearFlag(USART2, USART_FLAG_ORE);                                   // 清除溢出标志（否则DMA搬运异常）
        DMA_Cmd(DMA1_Channel6, ENABLE);                                            // 重启DMA
        if (len > 0)
            serial_on_receive(data, len);               // 处理收到的数据
        USART_ClearITPendingBit(USART2, USART_IT_IDLE); // 清除中断挂起标志
    }
}
...
void serial_send(uint8_t *data, uint16_t len)
{
    ...
    if (DMA1_Channel7->CCR & DMA_CCR_EN)                  // DMA开启
        while (DMA_GetFlagStatus(DMA1_FLAG_TC7) == RESET) // 阻塞，等待DMA发送完毕
            ;
    DMA_Cmd(DMA1_Channel7, DISABLE);            // 关闭DMA "in noncircular mode","The DMA channel must be disabled to reload a new number of data items into the DMA_CNDTRx register."
    DMA_ClearFlag(DMA1_FLAG_TC7);               // 清除传输完成标志
    DMA1_Channel7->CMAR = (uint32_t)data;       // 内存地址
    DMA_SetCurrDataCounter(DMA1_Channel7, len); // 发送字节数
    DMA_Cmd(DMA1_Channel7, ENABLE);             // 启用
}

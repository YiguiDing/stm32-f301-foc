// https://github.com/Marus/cortex-debug/blob/master/support/swo-init.c
#include "swo.h"
...
void swo_init(uint32_t portMask, uint32_t cpuCoreFreqHz, uint32_t baudrate)
{
	...
	// STM32F3 specific: configure DBGMCU for trace
	DBGMCU->CR |= DBGMCU_CR_TRACE_IOEN;  // Enable TRACE_IOEN for SWO pin output

	// Configure SWO pin (PB3 or PB14 for STM32F301, using PB3 with AF0)
	...
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource3, GPIO_AF_0);  // AF0 = SWO
...
void swo_sendchar(size_t const port, uint8_t ch)
{
	if (((ITM->TCR & ITM_TCR_ITMENA_Msk)) && /* ITM enabled */
		((ITM->TER & (1 << port))))			 /* ITM Port enabled */
	{
		// Wait until STIM is ready
